...
function App() {
    ...
    function handleDelete(handle) {
        const updatedHistory = uploadHistory.filter((upload) => upload.handle !== handle);
                    ...
                    <ul>
                        {uploadHistory.map((upload) => (
                            <li key={upload.handle}>
                                ...
                                <button className="upload-history-button" onClick={() => window.open(`https://cdn.filestackcontent.com/${upload.handle}`)}>
                                    View
                                </button>
                                <button className="delete-history-button" onClick={() => handleDelete(upload.handle)}>
                                    Delete
                                </button>
                            </li>
                        ))}
